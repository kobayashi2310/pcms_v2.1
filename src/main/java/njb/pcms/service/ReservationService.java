...
import njb.pcms.constant.PeriodConstants;
...
public class ReservationService {
    ...
    private final ReservationGroupingHelper reservationGroupingHelper;
    ...
    public List<ReservationGroupDto> getGroupedReservationsByDate(LocalDate date) {
        List<Reservation> reservations = reservationRepository
                .findByDateOrderByPc_IdAscUser_IdAscPeriod_PeriodAsc(date);
        return reservationGroupingHelper.groupReservations(reservations);
    }
    ...
    public Map<Long, Set<Byte>> getBookedPcsAndPeriodsForDate(LocalDate date) {
        ...
        List<Transport> activeTransportsOnDate = transportRepository
                .findByStatusAndCreatedAtLessThanEqualAndExpectedReturnDateGreaterThanEqual(
                        Transport.TransportStatus.IN_PROGRESS,
                        LocalDateTime.parse(date + "T00:00:00", DateTimeFormatter.ISO_LOCAL_DATE_TIME),
                        date);

        Set<Byte> allPeriods = PeriodConstants.ALL_PERIODS;
    ...
    public List<ReservationGroupDto> findGroupedReservationsByStudentId(String studentId) {
        ...
        return reservationGroupingHelper.groupReservations(userReservations);
    }
    ...
    public void approveReservations(List<Long> reservationIds) {
        ...
        for (Long id : reservationIds) {
            ...
            reservationsToApprove.add(reservation);
        }
        reservationRepository.saveAll(reservationsToApprove);
    }
