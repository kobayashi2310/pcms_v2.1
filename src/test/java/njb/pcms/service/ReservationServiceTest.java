...
class ReservationServiceTest {
    ...
    void setUp() {
        ...
        reservationService = new ReservationService(reservationRepository, userRepository, pcRepository, periodRepository);
    }
    ...
    private Reservation reservation(long id, User u, Pc p, LocalDate d, Period per, Reservation.ReservationStatus status) {
    ...
    void getBookedPcsAndPeriodsForDate() {
        ...
        Map<Long, Set<Byte>> map = reservationService.getBookedPcsAndPeriodsForDate(date);
    ...
    void getGroupedReservationsByDate() {
        ...
        List<ReservationGroupDto> groups = reservationService.getGroupedReservationsByDate(date);
    ...
    class CreateReservation {
        ...
        void create_ok() {
            ...
            when(reservationRepository.existsByPc_IdAndDateAndPeriod_Period(eq(10L), any(), eq((byte)1))).thenReturn(false);
            when(reservationRepository.existsByPc_IdAndDateAndPeriod_Period(eq(10L), any(), eq((byte)2))).thenReturn(false);

            ReservationRequestDto dto = new ReservationRequestDto();
            dto.setPcId(10L);
            dto.setDate(LocalDate.now());
            dto.setPeriodIds(new ArrayList<>(List.of((byte)2, (byte)1))); // out of order; service sorts
            dto.setReason("study");
        ...
        void create_noPeriods() {
            when(userRepository.findByStudentId("S1")).thenReturn(Optional.of(user(1, "S1", "U1")));
        ...
        void create_alreadyBooked() {
            ...
            Period per1 = period((byte)1, "1");
            when(periodRepository.findById((byte)1)).thenReturn(Optional.of(per1));
            when(reservationRepository.existsByPc_IdAndDateAndPeriod_Period(eq(10L), any(), eq((byte)1))).thenReturn(true);
        ...
        void create_missingEntities() {
            // user missing
            assertThrows(IllegalArgumentException.class, () -> reservationService.createReservation(new ReservationRequestDto(), "SXXX"));
            ...
            dto.setDate(LocalDate.now());
            dto.setReason("r");
            assertThrows(IllegalArgumentException.class, () -> reservationService.createReservation(dto, "S1"));
    ...
    @Test
    @DisplayName("findGroupedReservationsByStudentId returns groups for user reservations")
    void findGroupedReservationsByStudentId() {
        ...
        List<ReservationGroupDto> groups = reservationService.findGroupedReservationsByStudentId("S1");
    ...
    void findGroupedReservationsByStudentId_missingUser() {
        assertThrows(UsernameNotFoundException.class, () -> reservationService.findGroupedReservationsByStudentId("NOPE"));
    }
    ...
    void reportReturnByStudent_errors() {
        ...
        assertThrows(SecurityException.class, () -> reservationService.reportReturnByStudent(dto, "S1"));

        // not approved yet
        r.setUser(user(1, "S1", "U1"));
        r.setStatus(PENDING_APPROVAL);
        assertThrows(IllegalStateException.class, () -> reservationService.reportReturnByStudent(dto, "S1"));
    }
    ...
    void cancelReservationsByStudent() {
        ...
        Pc p = pc(10, "SN");
        Period per1 = period((byte)1, "1");
        Reservation r = reservation(1, u, p, LocalDate.now(), per1, PENDING_APPROVAL);
        when(reservationRepository.findById(1L)).thenReturn(Optional.of(r));
    ...
    void cancelReservationsByStudent_errors() {
        assertThrows(IllegalArgumentException.class, () -> reservationService.cancelReservationsByStudent(Collections.emptyList(), "S1"));

        User other = user(2, "S2", "U2");
        Reservation r = reservation(1, other, pc(10, "SN"), LocalDate.now(), period((byte)1, "1"), PENDING_APPROVAL);
        when(reservationRepository.findById(1L)).thenReturn(Optional.of(r));
        assertThrows(UsernameNotFoundException.class, () -> reservationService.cancelReservationsByStudent(List.of(1L), "S1"));

        r.setUser(user(1, "S1", "U1"));
        r.setStatus(APPROVED);
        assertThrows(IllegalStateException.class, () -> reservationService.cancelReservationsByStudent(List.of(1L), "S1"));
    }
    ...
    void getPendingReservations() {
        ...
        List<ReservationGroupDto> groups = reservationService.getPendingReservations();
        assertEquals(1, groups.size());
    }
    ...
    void approveReservations() {
        Reservation r1 = reservation(1, user(1, "S1", "U1"), pc(10, "SN"), LocalDate.now(), period((byte)1, "1"), PENDING_APPROVAL);
    ...
    void approveReservations_errors() {
        assertThrows(IllegalArgumentException.class, () -> reservationService.approveReservations(Collections.emptyList()));

        Reservation r = reservation(1, user(1, "S1", "U1"), pc(10, "SN"), LocalDate.now(), period((byte)1, "1"), APPROVED);
        when(reservationRepository.findById(1L)).thenReturn(Optional.of(r));
        assertThrows(IllegalStateException.class, () -> reservationService.approveReservations(List.of(1L)));
    }

    @Test
    @DisplayName("denyReservations deletes pending ones and validates inputs/state")
    void denyReservations() {
        assertThrows(IllegalArgumentException.class, () -> reservationService.denyReservations(Collections.emptyList()));

        Reservation r = reservation(1, user(1, "S1", "U1"), pc(10, "SN"), LocalDate.now(), period((byte)1, "1"), PENDING_APPROVAL);
        when(reservationRepository.findById(1L)).thenReturn(Optional.of(r));
        ...
        r.setStatus(APPROVED);
        assertThrows(IllegalStateException.class, () -> reservationService.denyReservations(List.of(1L)));
    }
}
